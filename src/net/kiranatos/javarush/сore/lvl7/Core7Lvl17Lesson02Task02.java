...
2. Создай public static нить SortThread, которая в методе run 
отсортирует статический массив testArray используя метод sort 
Требования:
...
public class Core7Lvl17Lesson02Task02 {
    public static int countThreads = 10;
    public static int[] testArray = new int[1000];

    static {
        for (int i = 0; i < Core7Lvl17Lesson02Task02.testArray.length; i++) {       
            testArray[i] = i;       
        }
    }
