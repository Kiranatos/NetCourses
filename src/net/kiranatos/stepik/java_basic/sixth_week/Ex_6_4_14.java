 ...
 * 
 * В приведенном коде используется оператор assert. Этот оператор используется 
 * для того, чтобы проверять определенные инварианты в коде. 
 * С помощью него возможно писать небольшие тесты и следить за корректностью 
 * своей программы (в обычной ситуации предпочтительно для этих целей 
 * использовать библиотеки для модульного тестирования, которые выходят 
 ...
 * то в программе возникает исключение AssertionError 
 ...
 * В случае IntellijIDEA, например, эта опция указывается в 
 * поле Run -> Edit Configurations... -> конфигурация запуска вашей 
 * программы -> VM Options.
 * 
 * Код, который необходимо заставить успешно работать:
 * 
 * В конечном итоге, вам нужно реализовать классы 
 * MailService, MailMessage и Salary 
 * (и, вероятно, вспомогательные классы и интерфейсы) и отправить их код в форму. 
 * Все классы должны быть публичными и статическими (ваши классы подставятся 
 * во внешний класс для тестирования).
 * 
 * В идеологически правильном решении не должно фигурировать 
 * ни одного оператора instanceof.
 * 
 * В классе для тестирования объявлены следующие импорты:
 */
public class Ex_6_4_14 {
    
    public static interface Sendable<T> { String getFrom(); String getTo();  T getContent(); }
    ...
    public static class MailService<T> implements Consumer<Sendable<T>> { 
        ...
        public void accept(Sendable<T> t) throws NullPointerException {                        
            List<T> content = mailBox.get(t.getTo());
            content.add(t.getContent());
            mailBox.put(t.getTo(), content);
        } 
...
*/
   ...
   // public static interface Sendable<T> {}
    ...
    //public static class BoxMap<K, T> extends HashMap<K, List<T>>   {}
