package net.kiranatos.epam.summer20.p6;

public class DescriptionP6 { }
/* Practical task №6
_______________________
Notes.
(1) Everywhere in the code, when implementing functional interfaces, use lambda 
expressions or method references.
(2) Remember that the containers HashSet/ТгееSet do not track potential 
occurrence of duplicates on changing the state of the objects that these 
containers store. The same concerns the keys of HashMap/ТгееМар containers.
(3) In the task 6, a ‘word’ should be considered a continuous sequence of Latin 
letters.
_______________________
...
Classes:
Part1 - contains mock of the console input / output and call of the method 
WordContainer.main
WordContainer - contains the required functionality 
Word - is set in correspondence to a model ‘a word with information about its 
frequency in the text’
———————————————————————————————-
Develop an application that reads text from console input and displays words 
in decreasing order of frequency of their occurrence in the text (if the 
frequencies coincide, the order is lexicographical). A ‘word’ should be 
considered a continuous sequence of non-whitespace characters.
Solve the task using the OOP approach.
The Word class, which contains a string field named ‘content’ and the integer 
field named ‘frequency’, the WordContainer container aggregates Word objects.
When using container core classes, correctly implement (in case of need) 
the methods Word#equals / Word#hashCode / Word#compareTo.
When starting, WordContainer.main reads from the standard input stream (console). 
Console input generally may contain several lines, each line may contain several 
words. The word ‘stop’ is a sign of console input end.
...
When counting in a circle, each k-th person (0 <k <n) is crossed out until only 
one is left.
...
——————————————————
Compare the run time of each method on the same input data (set in Part2.main) 
using the ArrayList and LinkedList objects. 
...
If there is no free place up to the end of the parking, then the free place is 
searched from the beginning of the parking. 
...
Parking state after the methods are executed (the value returned by the methods 
is indicated on the right, after comma):
——————————————————
...
Implement the iterator so that it iterated over the numbers within this range 
...
public class Tree<E extends Comparable<E>> {  
 ...
 //  adds all elements from the array to the container (call in the ‘add’ method 
loop, see above)
 ...
 // IMPORTANT! when deleting an element, the tree should not lose the feature of 
...
1. If the node does not have child nodes, then it’s needed to replace the 
corresponding pointer with null in its parent.
2. If the node has only one child node, then you need to create a new 
relationship between the parent of the node to be deleted and its child node. 
3. If the node has two child nodes, then you need to 
    3.1. find the element following it (this element will not have a right descendant)  
    3.2. place its left descendant in the place of the found element  
    3.3. replace the deleted node with the found node.
...
The format of the command line input parameters for the application 
(the program should understand both short and long options):
——————————————————
-i (or --input) path to the input file; 
-t (or --task) the name of the subtask.
——————————————————
There are 4 command line parameters in total, i.e. two pairs 
<option_name option_value>. The order of options with their values is arbitrary.
...
———————————————————————————————-
In the input file, find the three words that occur most often 
(if the frequencies coincide, choose that that occurs earlier), and print 
them sorted alphabetically in reverse order, in the format: word ==> frequency
Output example:
——————————————————
panda ==> 15 
ezhik ==> 20 
...
word ==> the quantity of the letters in the word. The list should be sorted in 
descending order of the letters quantity in the word.
If two words have the same quantity of the letters, then the word that occurred 
in the source file earlier should also be earlier in the resulting list.
Output example:
——————————————————
anesthetist ==> 11
kitchen ==> 7 
...
———————————————————————————————-
In the input file, find the first three words that have duplicates, and print 
their inversion in upper case.
Output example 
——————————————————
ADNAP 
TAC 
